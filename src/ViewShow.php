...
trait ViewShow
{
    ...
    public function show($data)
    {
        ...
        if (isset($this->_data['page']['css'])) {
            $this->_body = str_replace("</head>", "<link href=\"".$this->_data['page']['css']."\" rel=\"stylesheet\"></head>", $this->_body);
        }

        // 머리말 커스텀 javascript 파일이 있는 경우
        if (isset($this->_data['page']['script'])) {
            $this->_body = str_replace("</html>", "<script src=\"".$this->_data['page']['script']."\"></script></html>", $this->_body);
        }
